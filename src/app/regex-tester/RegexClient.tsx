...
const FLAG_OPTIONS = [
  ...
  { flag: "s", label: "s", title: "Dot matches newline" },
];
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>Regular Expression</label>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm" style={{ color: "var(--text-muted)" }}>Flags:</span>
        {FLAG_OPTIONS.map((f) => (
        ...
        <span className="text-xs ml-2" style={{ color: "var(--text-muted)" }}>
          {matches.length > 0 ? <span style={{ color: "var(--success)" }}>{matches.length} match{matches.length !== 1 ? "es" : ""}</span> : pattern && !error ? <span style={{ color: "var(--error)" }}>No matches</span> : ""}
        </span>
      </div>
      ...
      {/* Test text */}
      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>Test Text</label>
        <textarea
          value={text}
      ...
      {highlighted && (
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>Matches highlighted</label>
      ...
      {matches.length > 0 && (
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium" style={{ color: "var(--text-muted)" }}>Match details</label>
          <div className="rounded-lg overflow-hidden" style={{ border: "1px solid var(--border)" }}>
            {matches.slice(0, 20).map((m, i) => (
              <div key={i} className="flex items-center gap-4 px-4 py-2 text-sm" style={{ borderTop: i > 0 ? "1px solid var(--border)" : "none" }}>
                ...
                <span className="text-xs ml-auto" style={{ color: "var(--text-muted)" }}>index {m.index}</span>
              </div>
            ))}
            {matches.length > 20 && <div className="px-4 py-2 text-xs" style={{ color: "var(--text-muted)", borderTop: "1px solid var(--border)" }}>…and {matches.length - 20} more matches</div>}
          </div>
        </div>
      )}
    </div>
